<template>
  ...
  <div>
    <el-select
      class="select"
      clearable
      placeholder="请选择需要签到的机台"
      @change="checkIn($event)"
    >
      <el-option
        v-for="list in machineList"
...
export default {
  ...
  methods: {
    ...
    //点击选择获取机台名称，并将机台存放入localstorage，方便其他模块根据其调取数据
    getDevice(item) {
      // alert(item)
