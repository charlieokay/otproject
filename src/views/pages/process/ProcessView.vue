      ...
      >
        <el-card :body-style="{padding: '0px'}">
          ...
          <div style="padding: 14px">
            ...
            ><br />
            <span>设备名称:{{ list["devices_name"] }}</span
            ><br />
            <span>登录账号:{{ list["tenant_id"] }}</span
            ><br />

            <span>文件名:{{ list["file_name"] }}</span
            ><br />
            <span>文件地址:{{ list["file_path"] }}</span
            ><br />
            <span>文档类型字段:{{ list["resource_type"] }}</span
            ><br />
            <span>文档类型:{{ list["resource_type_str"] }}</span
            ><br />
            <div class="bottom">
              <time class="time">{{}}</time>
              <el-button text class="button">预览</el-button>
            </div>
          </div>
        </el-card>
      </el-col>
...
export default {
  ...
  methods: {
    getprocess() {
      axios
        .get(
          "/api/public//productSchedule/standard/?page_index=1&show_count=16",
          {
            // responseType: 'blob',
            headers: {
        ...
        .then((res) => {
          ...
          data_get.forEach((item) => {
            ...
            if (getDevice === devices_name) {
              ...
              item?.custom_data_resource_columns?.processes.forEach((item1) => {
